...
int elegir_destino(const Destino *destinos, int n) {
    ...
    // 1) Contar conocidos vs desconocidos y calcular valores necesarios
    for (int i = 0; i < n; i++) {
        if (destinos[i].es_conocido) {
            conocidos++;
    ...
    // 3) Calcular media de los costos conocidos
    double media = suma_conocidos / conocidos;
    
    // 4) Encontrar el destino más cercano a la media
    int indice_elegido = -1;
    double distancia_minima = -1.0;
    
    for (int i = 0; i < n; i++) {
        double valor_actual;
        
        // Asignar valor representativo según Regla 4
        if (destinos[i].es_conocido) {
            valor_actual = (double)destinos[i].costo;
        } else {
            // Desconocidos son mayores que cualquier conocido (Regla 4)
            valor_actual = (double)(max_conocido + 1);
        }
        
        double distancia = fabs(valor_actual - media);
        
        // Encontrar el más cercano (en caso de empate, se queda con el primero)
        if (indice_elegido == -1 || distancia < distancia_minima) {
            indice_elegido = i;
...
int main(void) {
    int n;
    printf("Ingresa el número de destinos posibles: ");
    if (scanf("%d", &n) != 1 || n < 3) {
